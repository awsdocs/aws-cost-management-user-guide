...
1. In the `copy` command, replace *<AWS\_ROLE>* with the ARN of an IAM role that has permissions to access the Amazon S3 bucket where you store your AWS Cost and Usage reports, and replace *<S3\_BUCKET\_REGION>* with the region that your Amazon S3 bucket is in \(e\.g\., `us-east-1`\)\.
