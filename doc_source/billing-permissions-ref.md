...
This topic provides examples of several identity\-based policies\. These policies that demonstrate how an account administrator attaches permissions policies to IAM identities \(users, groups, and roles\) to grant permissions for performing operations on Billing and Cost Management resources\.
...
| budgets:CreateBudgetAction |  Allow or deny IAM users permission to create a budget action, which is an action that AWS Budgets can take on your behalf if your budget exceeds a specific cost or usage threshold\. For more information, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:DeleteBudgetAction |  Allow or deny IAM users permission to delete a budget action\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:ExecuteBudgetAction |  Allow or deny IAM users permission to initiate a pending budget action and reverse a previous budget action\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:UpdateBudgetAction |  Allow or deny IAM users permission to update the settings of an existing budget action\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:DescribeBudgetAction |  Allow or deny IAM users permission to retrieve the details of a budget action\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:DescribeBudgetActionHistories |  Allow or deny IAM users permission to retrieve the historical statuses of a budget action\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:DescribeBudgetActionsForAccount |  Allow or deny IAM users permission to retrieve the details of all budget actions associated with your account\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
| budgets:DescribeBudgetActionsForBudget |  Allow or deny IAM users permission to retrieve the details of all budget actions associated with a budget\. To learn more about budget actions, see [Configuring AWS Budgets actions](budgets-controls.md)\.  | 
...
This managed policy is focused on the user\. It provides you with the permissions to grant permission to AWS Budgets to run the defined actions\. This policy provides full access to AWS Budgets\. This includes budgets actions to retrieve the status of your policies and run AWS resources using the AWS Management Console\.
