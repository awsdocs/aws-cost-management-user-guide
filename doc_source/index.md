# AWS Billing and Cost Management User Guide

-----
*****Copyright &copy; 2017 Amazon Web Services, Inc. and/or its affiliates. All rights reserved.*****

-----
Amazon's trademarks and trade dress may not be used in 
     connection with any product or service that is not Amazon's, 
     in any manner that is likely to cause confusion among customers, 
     or in any manner that disparages or discredits Amazon. All other 
     trademarks not owned by Amazon are the property of their respective
     owners, who may or may not be affiliated with, connected to, or 
     sponsored by Amazon.

-----
## Contents
+ [What Is AWS Billing and Cost Management?](billing-what-is.md)
+ [Getting Started](billing-getting-started.md)
+ [Using the Free Tier](billing-free-tier.md)
   + [Eligibility for the Free Tier](free-tier-eligibility.md)
   + [Free Tier Limits](free-tier-limits.md)
   + [Tracking Your AWS Free Tier Usage](tracking-free-tier-usage.md)
+ [Viewing Your Bill](getting-viewing-bill.md)
   + [Viewing Your Monthly Charges](invoice.md)
   + [Getting an Invoice Emailed to You](emailed-invoice.md)
+ [Understanding Your Usage with Billing Reports](billing-reports.md)
   + [Cost and Usage Report](billing-reports-costusage.md)
      + [Getting Started](billing-reports-gettingstarted.md)
         + [Controlling Access to Your AWS Cost and Usage Report Files](billing-reports-gettingstarted-access.md)
         + [Turning On the AWS Cost and Usage report](billing-reports-gettingstarted-turnonreports.md)
         + [Viewing Your AWS Cost and Usage Reports](download-billing-data-report.md)
         + [Setting Up an Amazon S3 Bucket for AWS Cost and Usage Reports](billing-reports-gettingstarted-s3.md)
         + [Viewing Cost and Usage Report Files in Amazon S3](billing-reports-costusage-files.md)
         + [Uploading an AWS Cost and Usage Report to Amazon Redshift](billing-reports-costusage-upload.md)
      + [Cost and Usage Report Details](billing-reports-costusage-details.md)
         + [Identity Details](enhanced-identity-columns.md)
         + [Billing Details](enhanced-billing-columns.md)
         + [Line Item Details](enhanced-lineitem-columns.md)
         + [Reservation Details](enhanced-reservation-columns.md)
         + [Pricing Details](pricing-columns.md)
         + [Product Details](enhanced-product-columns.md)
         + [Resource Tags](enhanced-resource-columns.md)
      + [Reserved Instances](billing-reports-costusage-ri.md)
         + [Reserved Instances](regular-reserved-instances.md)
         + [Region Reserved Instances](region-reserved-instances.md)
   + [Other Reports](billing-reports-other.md)
      + [Detailed Billing Report](DetailedBillingReport.md)
      + [Detailed Billing Report with Resources and Tags](reportstagsresources.md)
      + [Monthly Report](monthly-report.md)
      + [Monthly Cost Allocation Report](reportwithtags.md)
      + [Amazon EC2 Usage and Reserved Instance Utilization Reports](ec2reportlinks.md)
      + [AWS Usage Reports](view-usage-reports.md)
+ [Monitoring Your Usage and Costs](monitoring-costs.md)
   + [Reading Your Dashboard Graphs](view-billing-dashboard.md)
   + [Analyzing Your Costs with Cost Explorer](cost-explorer-what-is.md)
      + [Enabling Cost Explorer](cost-explorer-signup.md)
      + [Starting Cost Explorer](cost-explorer-start.md)
      + [Choosing Time Ranges for the Data That You Want to View](timerange.md)
      + [Forecasting with Cost Explorer](cost-forecast.md)
      + [Selecting a Style for Your Chart](chartstyle.md)
      + [Filtering the Data That You Want to View](selectdim.md)
      + [Grouping Data by Filter Type](groupdata.md)
      + [Choosing Advanced Options](advanced.md)
      + [Using Preconfigured Reports](preconfigreports.md)
      + [Creating and Saving Custom Reports](cost-explorer-new-report.md)
      + [Reading the Cost Explorer Chart](aboutchart.md)
      + [Reading the Cost Explorer Data Table](aboutdatatable.md)
      + [Downloading the CSV](downloadcsv.md)
      + [Saving Your Cost Explorer Configuration with Bookmarks or Favorites](savewithbookmarks.md)
      + [Managing Your Cost Explorer Reports](cost-explorer-saved-reports.md)
      + [Using the AWS Cost Explorer API](cost-explorer-api.md)
      + [Controlling Access for Cost Explorer](cost-explorer-access.md)
   + [Getting Reserved Instance Recommendations](ri-recommendations.md)
   + [Monitoring Charges with Alerts and Notifications](monitor-charges.md)
   + [Managing Your Costs with Budgets](budgets-managing-costs.md)
      + [Creating a Budget](budgets-create.md)
      + [Viewing Your Budgets](budgets-view.md)
      + [Editing a Budget](budgets-edit.md)
      + [Downloading a Budget](budgets-export.md)
      + [Copying a Budget](budgets-copy.md)
      + [Deleting a Budget](budgets-delete.md)
      + [Creating an Amazon SNS Topic for Budget Notifications](budgets-sns-policy.md)
   + [Using Cost Allocation Tags](cost-alloc-tags.md)
      + [AWS-Generated Cost Allocation Tags](aws-tags.md)
         + [Activating the AWS-Generated Cost Allocation Tag](activate-built-in-tags.md)
         + [Deactivating the AWS-Generated Cost Allocation Tag](deactivate-built-in-tags.md)
         + [AWS-Generated Cost Allocation Tag Restrictions](aws-tag-restrictions.md)
      + [User-Defined Cost Allocation Tags](custom-tags.md)
         + [Activating User-Defined Cost Allocation Tags](activating-tags.md)
         + [Refreshing User-Defined Cost Allocation Tags](refresh-cost-alloc-tags.md)
         + [User-Defined Tag Restrictions](allocation-tag-restrictions.md)
      + [Monthly Cost Allocation Report](configurecostallocreport.md)
   + [Using the AWS Price List Service](price-changes.md)
      + [Using the AWS Price List Service API](using-pelong.md)
      + [Using the AWS Price List API](using-ppslong.md)
         + [Finding Prices in an Offer File](procedures.md)
         + [Reading an Offer File](reading-an-offer.md)
         + [Reading the Offer Index File](reading-the-offer-index.md)
      + [Setting Up Change Notifications](price-notification.md)
   + [Avoiding Unexpected Charges](checklistforunwantedcharges.md)
+ [Managing Your Account](change-account-settings.md)
   + [Managing an AWS Account](manage-account-payment.md)
   + [Managing an Account in India](manage-account-payment-aispl.md)
      + [Switching to AISPL FAQ](aispl-switch-faq.md)
   + [Closing an Account](close-account.md)
+ [Managing Your Payments](manage-payments.md)
   + [Managing Your AWS Payments](edit-payment-method.md)
   + [Managing Your Payments in India](edit-aispl-payment-method.md)
+ [Controlling Access](control-access-billing.md)
   + [Granting Access to Your Billing Information and Tools](grantaccess.md)
   + [Billing and Cost Management Permissions Reference](billing-permissions-ref.md)
+ [Consolidated Billing for Organizations](consolidated-billing.md)
   + [Consolidated Billing Process](useconsolidatedbilling-procedure.md)
   + [Consolidated Billing in India](useconsolidatedbilling-India.md)
   + [Effective Billing Date](useconsolidatedbilling-effective.md)
   + [Billing and Account Activity](useconsolidatedbilling-activity.md)
   + [Volume Discounts](useconsolidatedbilling-discounts.md)
   + [AWS Credits](useconsolidatedbilling-credits.md)
   + [Reserved Instances](ri-behavior.md)
      + [Billing Examples for Specific Services](consolidatedbilling-other.md)
      + [Turning Off Reserved Instance Sharing](ri-turn-off.md)
   + [Understanding Consolidated Bills](con-bill-blended-rates.md)
   + [AWS Support Charges for Accounts in an Organization](consolidatedbilling-support.md)
+ [Limits](billing-limits.md)
+ [Contacting Customer Support About Your Bill](billing-get-answers.md)
+ [API Reference](api-reference.md)
   + [Actions](operations.md)
      + [CreateBudget](create-budget.md)
      + [CreateNotification](create-notification.md)
      + [CreateSubscriber](create-subscriber.md)
      + [DeleteBudget](delete-budget.md)
      + [DeleteNotification](delete-notification.md)
      + [DeleteReportDefinition](delete-report-definition.md)
      + [DeleteSubscriber](delete-subscriber.md)
      + [DescribeBudget](describe-budget.md)
      + [DescribeBudgets](list-budget.md)
      + [DescribeNotificationsForBudget](list-notification.md)
      + [DescribeReportDefinitions](describe-report-definitions.md)
      + [DescribeSubscribersForNotification](list-subscriber.md)
      + [PutReportDefinition](put-report-definition.md)
      + [UpdateBudget](update-budget.md)
      + [UpdateNotification](update-notification.md)
      + [UpdateSubscriber](update-subscriber.md)
   + [Data Types](data-types.md)
      + [Budget](data-type-budget.md)
      + [CalculatedSpend](data-type-calculated-spend.md)
      + [CostTypes](data-type-cost.md)
      + [CostFilter](data-type-filter.md)
      + [Notification](data-type-notification.md)
      + [NotificationWithsubscribers](data-type-notification-with-subscribers.md)
      + [ReportDefinition](report-definition.md)
      + [Spend](data-type-spend.md)
      + [Subscriber](data-type-subscriber.md)
+ [Document History](History.md)
+ [AWS Glossary](glossary.md)